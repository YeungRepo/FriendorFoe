...
im =  cv2.imread("../Data/Kernels/Caco2-Healthy_Repr30xP1.tif",-1);
#print(np.asarray(im))
#print(im_cv)
...
im_array_gray_Caco2 = im_array_gray;
...
im = cv2.imread("../Data/Kernels/Caco2-Unhealthy_Repr30xP1.tif",-1);
#imshow(np.asarray(im))
im_array_gray = im;#np.asarray(im,dtype=np.float32)
#im_array_gray = rgb2gray(im_array);
if invert_image:
...
im_array_gray_Caco2_UH = im_array_gray

if set_kernel:
    #Define Healthy A549s
    sample_kernel_healthy_Caco2 = np.array(im_array_gray_Caco2[300:490,250:400])
    sample_kernel_healthy_Caco2 = (np.mean(sample_kernel_healthy_Caco2,axis=None)-sample_kernel_healthy_Caco2)/(np.mean(sample_kernel_healthy_Caco2,axis=None))
    sample_kernel_healthy_Caco2[sample_kernel_healthy_Caco2<0.00] =0.0; 
    mask_sample_kernel_healthy_Caco2 = sample_kernel_healthy_Caco2;

    #Define Bacterial Patches
    sample_kernel_bacterial_patch = (im_array_gray_bacterial_patch[600:800,1350:1550]) #np.array(im_array_gray_bacterial_patch[625:795,350:550])#
    ...
    sample_kernel_unhealthy_Caco2 =np.array(im_array_gray_Caco2_UH[600:700,200:300])
    sample_kernel_unhealthy_Caco2 = (np.mean(sample_kernel_unhealthy_Caco2,axis=None)-sample_kernel_unhealthy_Caco2)/(np.mean(sample_kernel_unhealthy_Caco2,axis=None))
    sample_kernel_unhealthy_Caco2[sample_kernel_unhealthy_Caco2<0.00] =0.0; 
    mask_sample_kernel_unhealthy_Caco2 = sample_kernel_unhealthy_Caco2;
...
if show_kernels ==True:
    import matplotlib.pyplot as plt;
    plt.figure()
    plt.imshow(np.abs(sample_kernel_healthy_Caco2),cmap='gray')
    plt.title('Healthy Caco2  Kernel')
    ...
    plt.figure()
    plt.imshow(np.abs(sample_kernel_unhealthy_Caco2),cmap='gray')
    plt.title('Unhealthy Caco2')
...
list_of_kernels = [mask_sample_kernel_healthy_Caco2,mask_sample_kernel_unhealthy_Caco2,mask_sample_kernel_bacterial_patch]##[mask_sample_kernel_healthy_A549,mask_sample_kernel_healthy_dendritic,mask_sample_kernel_healthy_macrophage,mask_sample_kernel_unhealthy_A549,mask_sample_kernel_unhealthy_dendritic,mask_sample_kernel_unhealthy_macrophage,mask_sample_kernel_bacterial_patch]
...
#rootdir = '/Users/eyeung/Box/PNNL Friend or Foe Upload/20200611 A-549 P1 IV_V Pathogen NegCon #01-02/20200611 A-549 P1 IV_V Pathogen NegCon #01-02';
#rootdir = '/Users/eyeung/Box/PNNL Friend or Foe Upload/20200616 Caco-2 P1 IV_VPathogen NegCon #01-02/20200616 Caco-2 P1 IV_VPathogen NegCon #01-02';
rootdir = '/Users/eyeung/Box/PNNL Friend or Foe Upload/20200616 Caco-2 P2 IV_VPathogen #03-05/20200616 Caco-2 P2 IV_VPathogen #03-05';
...
Pickle_Path = '/Users/eyeung/Box/DARPAFoF UCSB Team Share/Repr_Pickles/';
all_files = [];
...
#all_plate_rows=['G']; #F5  bacterial patch 
#all_plate_cols = ['9']; #F5 is bacterial patch 
...
list_of_alphas = [6.3,8.5,12.1];#[3.75,13.0,5.1,8.5,8.2,11.25,4.06]
list_of_colors = ['g','r','b']
import pickle
row_res_masking = 10;
col_res_masking = 10;

kernel_names = ['hCaco2','uhCaco2','bp']
invert_image = False
...
for This_Pos_Flag in Pos_Flag:#This will be an iteration over elements like 'A1', 'A2', ... 
    ...
    file_list_suffixes = [elem for elem in all_files if This_Pos_Flag == elem.split('_')[0] and  (1<np.int(elem.split('_')[-1].strip('.tif'))<24) and (np.int(elem.split('_')[-1].strip('.tif'))%2 ==0) ]
    #print(file_list_suffixes)
    #file_list_suffixes = [elem for elem in all_orig_files if (This_Pos_Flag == elem.split('_')[0] and 1<np.int(elem.split('_')[-1].strip('.tif'))<41) ]
    ...
    if has_past_processing==True:
        ...
        with open(Pickle_Path + 'Caco2_Repr1-2.pickle','rb') as This_Feature_Dict_Pickle:
    ...
    for file_list_suffix in file_list_suffixes:
        ...
        im_array_gray = im_array_gray[150:650,150:650]
        ...
        for sample_kernel_ind in [0,1,2]:
                ...
                output_file = open(Pickle_Path+'Caco_Repr3-5.pickle','wb')
        ...
        if calibrate_kernels:
            ...
            kernel_names = ['Healthy Caco2','Unhealthy Caco2','Bacterial Patches'];
